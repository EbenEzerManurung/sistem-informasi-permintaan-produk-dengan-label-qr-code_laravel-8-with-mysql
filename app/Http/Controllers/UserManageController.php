...
class UserManageController extends Controller
{
    ...
    public function firstAccount(Request $req)
    {
        ...
        $access->permintaan_produk = 1;
        $access->permintaan_produksi = 1;
        $access->formpermintaan_produksi = 1;
        $access->data_produk = 1;
        $access->produk_masuk = 1;
        $access->validasi = 1;
        $access->mencetak_label= 1;
        $access->produk_keluar = 1;
        $access->save();
    ...
    public function createAccount(Request $req)
    {
        ...
        if($check_access->kelola_akun == 1){
        	...
        	if($req->foto != '' && $check_email == 0 && $check_username == 0)
        	{
                ...
                elseif($req->role == 'ppic'){
                    $access = new Acces;
                    $access->user = $users->id;
                    $access->kelola_akun = 0;
                    $access->permintaan_produk = 1;
                    $access->formpermintaan_produksi = 1;
                    $access->permintaan_produksi = 0;
                    $access->data_produk = 1;
                    $access->produk_masuk = 0;
                    $access->validasi = 1;
                    $access->mencetak_label= 0;
                    $access->produk_keluar = 1;
                   
                    $access->save();
                }
                elseif($req->role == 'warehousefg'){
                    $access = new Acces;
                    $access->user = $users->id;
                    $access->kelola_akun = 0;
                    $access->permintaan_produk = 0;
                    $access->formpermintaan_produksi = 0;
                    $access->permintaan_produksi = 0;
                    $access->data_produk = 1;
                    $access->produk_masuk = 0;
                    $access->validasi = 0;
                    $access->mencetak_label= 1;
                    $access->produk_keluar = 0;
